...
from services.png_to_jpg_service import png_to_jpg
from utils.file_cleanup import delete_file_now, delete_file_after_delay  # ✅ 추가
import os
from datetime import datetime

png_to_jpg_bp = Blueprint("png_to_jpg", __name__)

@png_to_jpg_bp.route("/convert/png-to-jpg", methods=["POST"])
def convert_png_to_jpg():
    ...
    # ✅ 삭제 처리
    delete_file_now(input_path)                # 업로드 PNG 즉시 삭제
    delete_file_after_delay(output_path, 60)   # 변환된 JPG 1분 후 삭제

    return jsonify({"success": True, "filename": output_filename})

@png_to_jpg_bp.route("/download/<filename>", methods=["GET"])
def download_png_to_jpg(filename):
